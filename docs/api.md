...
Returns the same shape as `GET`. Disarming finalizes any event that is currently recording with `finalize_reason: "disarmed"`, so nothing is left half-written, and it clears the latched motion so a sensor that is still reading high cannot immediately open a replacement event. That event's AI analysis and notification are skipped rather than queued, and queued retries for older events are skipped too, so a disarmed camera never calls a paid provider. Arming again while the sensor is still reading motion starts a new event straight away, because no fresh edge would otherwise arrive. The state is persisted to `/var/lib/securecam/arm-state.json` and survives restarts and power cuts. `400` if `armed` is not a boolean.
